...
class TestSMTQueryEngine(unittest.TestCase):
    ...
    def test_search(self, mock_embedding_service_class):
        ...
        mock_svc.search.return_value = [(symbol1, 0.95), (symbol2, 0.87)]

        results = self.engine.search("cycle detection", top_k=2)

        self.assertEqual(len(results.hits), 2)
        self.assertEqual(results.hits[0].name, "detect_cycles")
        self.assertEqual(results.hits[0].type, "Function")
        self.assertAlmostEqual(results.hits[0].score, 0.95)
        self.assertEqual(results.hits[1].name, "CycleGroup")
    ...
    def test_status(self, mock_validate):
        ...
        mock_validate.return_value = ValidationResult(
            is_fresh=True, git_head="a1b2c3d", graph_head="a1b2c3d", commits_behind=0,
        )
        self.mock_client.get_stats.return_value = {"node_count": 150, "edge_count": 200}
    ...
    def test_status_stale(self, mock_validate):
        ...
        mock_validate.return_value = ValidationResult(
            is_fresh=False, git_head="z9y8x7w", graph_head="a1b2c3d", commits_behind=3,
        )
        self.mock_client.get_stats.return_value = {"node_count": 100, "edge_count": 120}
